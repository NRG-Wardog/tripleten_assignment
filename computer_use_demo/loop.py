...
def _response_to_params(response) -> list[BetaContentBlockParam]:
    """
    Supports:
      - Anthropic BetaMessage objects (original path)
      - Nebius/Yandex FM JSON dict: {"result":{"alternatives":[{"message":{"text": ...}}]}}
    """
    # Anthropic BetaMessage path
    if hasattr(response, "content"):
        ...
        for block in response.content:
            if isinstance(block, BetaTextBlock):
                ...
                # handle dict-form blocks too
                if block.get("type") == "text":
                    res.append(BetaTextBlockParam(type="text", text=block.get("text","")))
                elif block.get("type") == "tool_use":
                    res.append(cast(BetaToolUseBlockParam, block))
            else:
                # anthropic tool blocks
                res.append(cast(BetaToolUseBlockParam, block.model_dump()))
        return res

    # Nebius dict fallback
    if isinstance(response, dict):
        ...
        return [BetaTextBlockParam(type="text", text=text or "")]

    # Last resort: just stringify
    return [BetaTextBlockParam(type="text", text=str(response))]
