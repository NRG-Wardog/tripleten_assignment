...
PROVIDER_TO_DEFAULT_MODEL_NAME: dict[APIProvider, str] = {
    ...
    APIProvider.NEBIUS: "deepseek-ai/DeepSeek-R1-0528"
}
...
def setup_state():
    ...
    if "provider" not in st.session_state:
        env_provider = (os.getenv("API_PROVIDER") or "anthropic").lower().strip()
        try:
    ...
    if "provider_radio" not in st.session_state:
        # keep radio’s value the SAME TYPE as options (enum)
    ...
    with st.sidebar:
        ...
        def _reset_api_provider():
            if st.session_state.provider_radio != st.session_state.provider:
                st.session_state.provider = st.session_state.provider_radio  # enum
                _reset_model()
                st.session_state.auth_validated = False

        provider_options = list(APIProvider)
        st.radio(
            "API Provider",
            options=provider_options,
            key="provider_radio",
            format_func=lambda opt: opt.value.title(),
